  ...
  const handleSearch = async () => {
    ...
    try {
      if (appointmentType === 'doctor') {
        ...
        const result = await appointmentsService.searchHospitals({
          name: searchQuery,
          location: '',
        });
        setHospitals(result);
      } else if (appointmentType === 'lab') {
        const result = await appointmentsService.searchLabs({
          name: searchQuery,
          location: '',
        });
        setLabs(result);
        const tests = await appointmentsService.getLabTests();
        setLabTests(tests);
      }
      setStep(3);
    } catch (error) {
    ...
    try {
      ...
      if (appointmentType === 'doctor' && selectedDoctor && selectedSlot) {
        ...
        request = {
          ...
          preferredDate: selectedDate || new Date().toISOString().split('T')[0],
          homeCollection,
          patientName,
          patientAge: parseInt(patientAge),
          patientContact,
      ...
      Alert.alert(
        ...
        `Appointment booked successfully!\n\nBooking ID: ${response.bookingId}\n${
          response.tokenNumber ? `Token: ${response.tokenNumber}` : ''
        }`,
        [
          {
            text: 'OK',
          ...
          <ScrollView>
            {doctors.map(doctor => (
              ...
              >
                ...
                <Text style={styles.providerSpecialty}>{doctor.specialty}</Text>
