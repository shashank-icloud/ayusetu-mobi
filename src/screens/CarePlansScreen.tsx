...
type Props = NativeStackScreenProps<RootStackParamList, 'CarePlans'>;
...
const CarePlansScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const handleActivityToggle = async (activity: CarePlanActivity) => {
        ...
        try {
            const newStatus: CarePlanActivity['status'] = activity.status === 'completed' ? 'pending' : 'completed';
            await appointmentsService.updateActivityStatus(selectedPlan.id, activity.id, newStatus);
            ...
            const updatedPlan = {
                ...
                activities: selectedPlan.activities.map(a =>
                    a.id === activity.id
                        ? { ...a, status: newStatus as CarePlanActivity['status'], completedDate: newStatus === 'completed' ? new Date().toISOString() : undefined }
                        : a
                ),
            };

            // Recalculate progress
            const completedCount = updatedPlan.activities.filter(a => a.status === 'completed').length;
            updatedPlan.progressPercentage = Math.round((completedCount / updatedPlan.activities.length) * 100);

            setSelectedPlan(updatedPlan as CarePlan);

            // Update in list
            setCarePlans(carePlans.map(p => (p.id === updatedPlan.id ? updatedPlan as CarePlan : p)));
        } catch (error) {
                    ...
                    <View key={goal.id} style={styles.goalCard}>
                        ...
                        <View style={styles.goalMetrics}>
                            <View style={styles.metric}>
                                ...
                                <Text style={styles.metricValue}>{goal.targetValue} {goal.unit}</Text>
                            </View>
                            <View style={styles.metric}>
                                <Text style={styles.metricLabel}>Current</Text>
                                <Text style={styles.metricValue}>{goal.currentValue} {goal.unit}</Text>
                            </View>
                            <View style={styles.metric}>
                                <Text style={styles.metricLabel}>Progress</Text>
                                <Text style={[styles.metricValue, { color: getProgressColor(goal.targetValue && goal.currentValue ? Math.round((goal.currentValue / goal.targetValue) * 100) : 0) }]}>
                                    {goal.targetValue && goal.currentValue ? Math.round((goal.currentValue / goal.targetValue) * 100) : 0}%
                                </Text>
                            </View>
                        </View>
                        <View style={styles.progressBar}>
                            <View
                                style={[
                                    styles.progressFill,
                                    {
                                        width: `${goal.targetValue && goal.currentValue ? Math.round((goal.currentValue / goal.targetValue) * 100) : 0}%`,
                                        backgroundColor: getProgressColor(goal.targetValue && goal.currentValue ? Math.round((goal.currentValue / goal.targetValue) * 100) : 0),
                                    },
                                ]}
                            />
                        </View>
                ...
                {Object.entries(groupedActivities).map(([frequency, activities]) => (
                    <View key={frequency} style={styles.frequencyGroup}>
                        ...
                        {activities.map(activity => (
                            <TouchableOpacity
                                key={activity.id}
                                style={[styles.activityCard, activity.status === 'completed' && styles.activityCardCompleted]}
                                onPress={() => handleActivityToggle(activity)}
                            >
                                <View style={styles.activityHeader}>
                                    <View style={styles.checkbox}>
                                        <Text style={styles.checkboxText}>{activity.status === 'completed' ? '✅' : '⬜'}</Text>
                                    </View>
                                    <View style={styles.activityContent}>
                                        <Text style={[styles.activityTitle, activity.status === 'completed' && styles.activityTitleCompleted]}>
                                            {activity.title}
                                        </Text>
            ...
            <View style={styles.overviewCard}>
                ...
                <View style={styles.planMetrics}>
                    <View style={styles.planMetric}>
                        <Text style={styles.planMetricLabel}>Status</Text>
                        <Text style={styles.planMetricValue}>{getStatusLabel(selectedPlan.status)}</Text>
                    </View>
                    <View style={styles.planMetric}>
                        <Text style={styles.planMetricLabel}>Progress</Text>
                        <Text style={[styles.planMetricValue, { color: getProgressColor(selectedPlan.progressPercentage) }]}>
                    ...
                    <View style={styles.planMetric}>
                        ...
                        <Text style={styles.planMetricValue}>
                            {selectedPlan.activities.filter(a => a.status === 'completed').length}/{selectedPlan.activities.length}
                        </Text>
                    </View>
                </View>
                ...
                <View style={styles.dateRange}>
                    <Text style={styles.dateText}>
                        📅 {new Date(selectedPlan.startDate).toLocaleDateString()}{selectedPlan.endDate ? ` - ${new Date(selectedPlan.endDate).toLocaleDateString()}` : ''}
                    </Text>
                </View>
            </View>
        );
